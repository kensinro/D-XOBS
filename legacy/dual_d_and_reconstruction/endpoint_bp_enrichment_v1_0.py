# -*- coding: utf-8 -*-
"""
AIDO D-PHY + D-Clinical Endpoint BP-Enrichment Pipeline V1.0
============================================================

Endpoint version: designed for cancer-patient internal discriminability.
...
Purpose
-------
Run D-PHY + D-Clinical + BP-Enrichment + reconstructed BP-state audit for
clinically meaningful endpoints inside cancer patients, such as:

    1. TCGA-BRCA stage early vs late
    2. TCGA-BRCA lymph-node negative vs positive
    3. TCGA-BRCA survival/risk binary endpoint
    4. Custom binary endpoint table
    5. Positive-control tumor vs normal, only for sanity check

Core question
-------------
Can D-PHY + D-Clinical selected BP observables be reconstructed into coherent
BP-Enrichment modules that preserve patient-endpoint information and provide
oncogene / tumor-suppressor anchored mechanism interpretation?

Pipeline
--------
Raw gene expression
    ↓
Cancer-patient endpoint labels
    ↓
BP activity matrix
    ↓
D-PHY: local BP endpoint deviation
    ↓
D-Clinical: global endpoint contribution in BP space
    ↓
Dual-D BP candidate pool
    ↓
BP-Enrichment / BP-state module reconstruction
    ↓
Reconstructed state module activity
    ↓
Endpoint discriminability audit
    ↓
...
----------------
First:
    ENDPOINT_MODE = "tcga_stage_early_late"

Then:
    ENDPOINT_MODE = "tcga_node_negative_positive"

Optional:
    ENDPOINT_MODE = "custom_binary"

Positive-control only:
    ENDPOINT_MODE = "tcga_tumor_vs_normal"
...
OUT_DIR:
    ...
    13_endpoint_run_summary.json
    14_ENDPOINT_DPHY_DClinical_BPEnrichment_summary.xlsx
    figures/*.png
    README_ENDPOINT_DPHY_DClinical_BPEnrichment.txt
    ZIP package
"""
...
# Expression input: sample × gene or gene × sample are both supported.
GE_FILE = r"D:/AIDO-Data/UCSC_XENA/Breast Cancer (BRCA)/GE.tsv"

# Clinical / phenotype table.
# For TCGA-BRCA stage/node, use your clinical matrix if available.
# Example from previous upload:
...
# Endpoint mode:
#   "tcga_stage_early_late"
#   "tcga_node_negative_positive"
#   "tcga_survival_short_long"
#   "tcga_tumor_vs_normal"       positive control only
#   "custom_binary"
ENDPOINT_MODE = "tcga_stage_early_late"

# Output directory. Recommended: change when switching endpoint.
OUT_DIR = r"D:/AIDO-Temp/ENDPOINT_DPHY_DClinical_BPEnrichment_TCGA_Stage_V1"

# Custom binary label settings, only used if ENDPOINT_MODE = "custom_binary"
CUSTOM_LABEL_FILE = None
CUSTOM_SAMPLE_COL = None
CUSTOM_LABEL_COL = None

# For cancer-patient endpoint tasks, keep TCGA primary tumor only.
...
# Survival endpoint settings, used only for "tcga_survival_short_long".
# It creates a binary endpoint from available survival time and vital status:
#   short/poor = death event or time <= lower quantile
#   long/good  = alive/censored and time >= upper quantile
...
# "strict" is recommended for endpoint tasks.
...
# Reconstructed state classifier
...
def keep_tcga_primary_tumor_only(expr: pd.DataFrame) -> pd.DataFrame:
    ...
    print("[WARN] Could not identify enough TCGA primary tumor samples. Keeping all samples.")
    return expr
...
def read_clinical_table(path: str) -> pd.DataFrame:
    if path is None:
        raise ValueError("CLINICAL_FILE is required for this endpoint mode.")
    p = Path(path)
...
def infer_sample_column(df: pd.DataFrame) -> str:
    ...
    # Prefer first column if values look like TCGA samples/patients.
    for c in df.columns[:5]:
        vals = df[c].dropna().astype(str).head(20)
        if vals.str.contains("TCGA-", regex=False).mean() > 0.5:
            return c
...
def make_stage_labels(clinical: pd.DataFrame) -> pd.Series:
    ...
    for pid, val in s.items():
        if val in ["nan", "none", "", "not reported", "not available", "na"]:
            continue

        # Put stage IV before stage I to avoid matching stage i inside stage iv.
    ...
    if y.nunique() != 2 or len(y) < 30:
        raise ValueError(f"Stage label failed or too small. n={len(y)}, counts={y.value_counts().to_dict()}")
...
def make_node_labels(clinical: pd.DataFrame) -> pd.Series:
    ...
    y = pd.Series(labels, name="label").astype(int)
    if y.nunique() != 2 or len(y) < 30:
        raise ValueError(f"Node label failed or too small. n={len(y)}, counts={y.value_counts().to_dict()}")
...
def make_survival_labels(clinical: pd.DataFrame) -> pd.Series:
    ...
    # Try to derive time from death/followup columns if available.
    time = pd.to_numeric(cdf[time_col], errors="coerce")

    # If a days_to_death column is mostly missing, try follow-up.
    ...
    if status_col is not None:
        status_raw = cdf[status_col].astype(str).str.lower()
        event = status_raw.map(lambda x: 1 if ("dead" in x or x in ["1", "deceased", "death"]) else 0)
    else:
    ...
    y = pd.Series(labels, name="label").astype(int)
    if y.nunique() != 2 or len(y) < 30:
        raise ValueError(f"Survival label failed or too small. n={len(y)}, counts={y.value_counts().to_dict()}")
...
def load_custom_binary_labels(path: str, sample_col: Optional[str], label_col: Optional[str]) -> pd.Series:
    ...
    tmp["patient_barcode"] = tmp[sample_col].map(tcga_patient_barcode)
    ...
    if pd.api.types.is_numeric_dtype(raw):
        ...
        if len(uniq) != 2:
            raise ValueError(f"Numeric label column must have exactly two classes. Found: {uniq[:10]}")
        y01 = ynum.map({uniq[0]: 0, uniq[1]: 1})
    else:
        s = raw.astype(str).str.strip().str.lower()
        pos = {"1", "late", "late_like", "high", "positive", "node_positive", "node-positive",
               "poor", "case", "yes", "true", "bad", "stage_iii_iv", "advanced"}
        neg = {"0", "early", "early_like", "low", "negative", "node_negative", "node-negative",
               "good", "control", "no", "false", "stage_i_ii", "localized"}
    ...
    labels = pd.Series(y01.values, index=tmp["patient_barcode"].values, name="label").dropna().astype(int)
    labels = labels[~labels.index.duplicated(keep="first")]
    print("[INFO] Custom label counts:", labels.value_counts().to_dict())
    return labels


def build_endpoint_labels(expr: pd.DataFrame) -> pd.Series:
    if ENDPOINT_MODE == "tcga_tumor_vs_normal":
        return make_tcga_tumor_normal_labels(expr.index)

    if ENDPOINT_MODE == "custom_binary":
        if CUSTOM_LABEL_FILE is None:
            raise ValueError("CUSTOM_LABEL_FILE is required for ENDPOINT_MODE='custom_binary'.")
    ...
    if ENDPOINT_MODE == "tcga_stage_early_late":
        return make_stage_labels(clinical)

    if ENDPOINT_MODE == "tcga_node_negative_positive":
        return make_node_labels(clinical)

    if ENDPOINT_MODE == "tcga_survival_short_long":
...
def align_expression_endpoint(expr: pd.DataFrame, y_patient: pd.Series) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    # Patient-level endpoints: map expression samples to patient barcode.
    ...
    if expr.shape[0] < 20:
        raise ValueError("Too few expression samples matched to endpoint labels.")
    ...
    if y.nunique() != 2:
        raise ValueError("Endpoint labels after expression alignment do not contain two classes.")
...
def compute_dclinical(bp_mat: pd.DataFrame, y: pd.Series) -> Tuple[pd.DataFrame, Dict]:
    ...
    if n_splits < 2:
        raise ValueError("Not enough samples in the smaller class for cross-validation.")
...
# =============================================================================
# h-layer oncogene / TSG anchoring
# =============================================================================
...
# =============================================================================
# Reconstructed state and mechanism audit
# =============================================================================
...
def compute_module_activity(bp_mat: pd.DataFrame, module_members: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(data, index=bp_mat.index).dropna(axis=1, how="all")
...
def compute_state_discriminability(module_activity: pd.DataFrame, y: pd.Series,
    ...
    # Multivariate reconstructed-state classifier
    info = {}
...
def make_figures(out_dir: Path, dphy: pd.DataFrame, dclinical: pd.DataFrame, dual: pd.DataFrame,
    ...
    if not top.empty:
        ...
        plt.xlabel("|Cohen d|")
        plt.title("Top D-PHY endpoint BP deviations")
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig(fig_dir / "FIG01_top_DPHY_endpoint_BP_deviations.png", dpi=300)
        plt.close()
    ...
    if not top.empty:
        ...
        plt.title("Top D-Clinical endpoint BP contributors")
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig(fig_dir / "FIG02_top_DClinical_endpoint_BP_contributors.png", dpi=300)
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(fig_dir / "FIG03_DualD_endpoint_BP_evidence_space.png", dpi=300)
    plt.close()
    ...
    if state_disc is not None and not state_disc.empty:
        ...
        plt.yticks(yv, labels)
        plt.xlabel("Module endpoint information score")
        plt.title("Top reconstructed endpoint-state BP modules")
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig(fig_dir / "FIG04_top_reconstructed_endpoint_state_modules.png", dpi=300)
        plt.close()
    ...
    if mechanism_audit is not None and not mechanism_audit.empty:
        ...
        plt.title("Top mechanism-anchored endpoint BP modules")
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig(fig_dir / "FIG05_top_mechanism_anchored_endpoint_modules.png", dpi=300)
        plt.close()
    ...
    if vals:
        ...
        plt.tight_layout()
        plt.savefig(fig_dir / "FIG06_endpoint_AUC_comparison.png", dpi=300)
        plt.close()
...
def run_pipeline() -> Path:
    ...
    config = {
        ...
        "CUSTOM_LABEL_FILE": CUSTOM_LABEL_FILE,
        ...
        "note": "Endpoint D-PHY/D-Clinical BP-Enrichment only. No TTU or drug tasks."
    }
    ...
    if ENDPOINT_MODE != "tcga_tumor_vs_normal" and KEEP_TCGA_PRIMARY_TUMOR_ONLY:
        expr = keep_tcga_primary_tumor_only(expr)

    # Labels
    y_endpoint = build_endpoint_labels(expr)
    ...
    make_figures(out_dir, dphy, dclinical, dual, state_disc, mechanism_audit, dclinical_info, state_info)
    ...
    run_summary = {
        ...
        "interpretation": (
            "Endpoint D-PHY/D-Clinical run. This excludes TTU/drug tasks and tests whether BP-Enrichment "
            "reconstructed states preserve cancer-patient endpoint discriminability and oncogene/TSG mechanism anchoring."
        )
    }
    (out_dir / "13_endpoint_run_summary.json").write_text(json.dumps(run_summary, indent=2), encoding="utf-8")

    try:
        with pd.ExcelWriter(out_dir / "14_ENDPOINT_DPHY_DClinical_BPEnrichment_summary.xlsx", engine="openpyxl") as writer:
    ...
    readme = f"""AIDO Endpoint D-PHY + D-Clinical BP-Enrichment V1.0
======================================================

No TTU. No drug-target task.

Endpoint mode: {ENDPOINT_MODE}

Main purpose
------------
This pipeline tests whether D-PHY + D-Clinical selected BP observables can be
reconstructed into BP-Enrichment modules that preserve cancer-patient endpoint
discriminability and support oncogene/tumor-suppressor mechanism interpretation.
...
Notes
-----
AUC close to 0 can still be highly informative if the module is endpoint-negative
state-up. Use AUC distance = |AUC - 0.5| for direction-free discriminability.

This endpoint run should be interpreted as cancer-patient internal discriminability,
not tumor-normal positive-control, unless ENDPOINT_MODE is tcga_tumor_vs_normal.
"""
    (out_dir / "README_ENDPOINT_DPHY_DClinical_BPEnrichment.txt").write_text(readme, encoding="utf-8")

    zip_path = out_dir.parent / f"{out_dir.name}.zip"
    zip_dir(out_dir, zip_path)

    print("\n[DONE] Endpoint D-PHY + D-Clinical BP-Enrichment completed.")
