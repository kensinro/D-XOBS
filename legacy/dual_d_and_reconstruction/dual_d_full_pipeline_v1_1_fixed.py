# -*- coding: utf-8 -*-
"""
AIDO DUAL-D FULL PIPELINE V1.1
==============================

真正完整的一版：從 raw gene expression 重新開始，做到 DUAL-D、BP-Enrichment、
h-layer biological anchoring、drug-to-BP-module reversal prototype、TTU candidate scoring。

整體概念
--------
Cancerized state / bad state
    vs
Good state / normal state
...
D-module
    ...
    ↓
Dual-D selected BP pool
    ├─ Tier 1: D-PHY high + D-Clinical high
    ├─ Tier 2: D-PHY high only
    ├─ Tier 3: D-Clinical high only
    └─ Tier 4: weak/unselected
    ↓
BP-Enrichment / BP-state module reconstruction
    ↓
h-layer biological anchoring
    ├─ CIViC
    ├─ COSMIC
    └─ OncoKB
    ↓
Drug-to-BP-module reversal prototype
    ├─ drug-target overlap
    ├─ desired reversal direction
    ├─ action consistency if drug action column exists
    └─ risk / off-direction module flags
    ↓
TTU candidate scoring

重點
----
1. D 不是 final answer，而是一個 module。
2. D-module = D-PHY + D-Clinical。
3. D-module 選出的 BP 送入 BP-Enrichment，重構成 BP-state modules。
4. BP-state modules 再接 drug reversal / TTU scoring。
5. Drug layer V1 是 drug-target overlap prototype；之後可升級 LINCS/CMap signature reversal。
...
第一次建議先跑 TCGA tumor-vs-normal：
    GE_FILE = r"D:/AIDO-Data/UCSC_XENA/Breast Cancer (BRCA)/GE.tsv"
    TARGET_MODE = "tcga_tumor_vs_normal"

如果沒有 drug-target file，本程式不會失敗，會輸出 drug_target_template.csv。
之後把 drug-target table 填好，再設定 DRUG_TARGET_FILE 重新跑即可。

Drug target file 支援欄位
-------------------------
最少需要：
    drug, gene
...
action 可用：
    inhibit / inhibitor / antagonist / down / suppress / block
    activate / agonist / up / induce / restore
    unknown
...
OUT_DIR:
    ...
    09_BP_module_reversal_targets.csv
    10_drug_BP_module_overlap.csv
    11_TTU_candidate_drug_scores.csv
    12_TTU_drug_risk_modules.csv
    13_RUN_SUMMARY.json
    14_DUALD_FULL_summary.xlsx
    figures/*.png
    ZIP package
"""
...
# 若為 None，會自動在 D:/AIDO-Data 搜尋 c5.go.bp*.gmt / go.bp*.gmt。
BP_GENESET_FILE = None

# Output folder
OUT_DIR = r"D:/AIDO-Temp/DUAL_D_FULL_TCGA_TumorVsNormal_V1_1_FIXED"

# Target mode
# Current built-in:
#   "tcga_tumor_vs_normal"
# For custom binary labels:
#   "custom_binary"
TARGET_MODE = "tcga_tumor_vs_normal"

# Custom label file, only needed when TARGET_MODE = "custom_binary"
LABEL_FILE = None
SAMPLE_COL = None
LABEL_COL = None
...
# Drug target file.
# If None or missing, the pipeline still runs and outputs a template.
DRUG_TARGET_FILE = None
# Example:
# DRUG_TARGET_FILE = r"D:/AIDO-Data/DrugTargets/drug_target_table.tsv"

# BP gene-set filtering
MIN_GENES_PER_BP = 5
MAX_GENES_PER_BP = 500
MAX_BP_TERMS = None        # None = use all mapped BP; set e.g. 6000 if memory is limited

# D-PHY settings
TOP_K_DPHY = 50
DPHY_FDR_CUTOFF = 0.05
DPHY_ABS_D_CUTOFF = 0.30
BOOTSTRAP_N = 50           # Set 0 to skip; 50 is faster, 100 more stable
BOOTSTRAP_TOP_K = 50
...
# Drug / TTU scoring
MIN_DRUG_TARGET_OVERLAP = 1
TOP_N_DRUG_OUTPUT = 300
ACTION_MATCH_BONUS = 1.25
ACTION_MISMATCH_PENALTY = 0.50
UNKNOWN_ACTION_FACTOR = 0.85
RISK_PENALTY_PER_MODULE = 0.05
...
def safe_read_table(path: Path, **kwargs) -> pd.DataFrame:
    ...
    if suffix == ".gz":
        # pandas can infer compression; try delimiter detection by extension before .gz
        name = path.name.lower()
        if name.endswith(".tsv.gz") or name.endswith(".txt.gz"):
            return pd.read_csv(path, sep="\t", low_memory=False, compression="gzip", **kwargs)
        return pd.read_csv(path, low_memory=False, compression="gzip", **kwargs)
...
def q_rank_score(values: pd.Series, larger_better: bool = True) -> pd.Series:
    """
    Convert a numeric vector into a 0-1 percentile score.

    Important:
    pandas rank(ascending=True) gives the largest value the largest percentile.
    Therefore:
        larger_better=True  -> ascending=True
        larger_better=False -> ascending=False

    V1.0 used the opposite direction, which inverted some ranking-based scores.
    This V1.1 function fixes that.
    """
...
def filter_bp_gene_sets(
    ...
    if max_terms is not None and len(out) > max_terms:
        # Prefer larger mapped gene sets, then stable alphabetical order
...
def load_expression_matrix(ge_file: str) -> pd.DataFrame:
    ...
    # Gene × sample format if sample IDs are in columns.
...
def align_expr_labels(expr: pd.DataFrame, y: pd.Series) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    # TCGA barcode fallback using first 15 characters.
    expr2 = expr.copy()
    y2 = y.copy()
...
def build_biomarker_reference() -> Tuple[Set[str], Set[str], Set[str]]:
    ...
    print(f"[INFO] Biomarker/cancer genes: {len(cancer)} | oncogenes: {len(onc)} | TSG: {len(tsg)}")
    return cancer, onc, tsg
...
# =============================================================================
# Drug target / TTU layer
# =============================================================================

def normalize_action(x: str) -> str:
    s = str(x).lower().strip()
    if any(k in s for k in ["inhibit", "inhibitor", "antagonist", "down", "suppress", "block", "decrease", "negative"]):
        return "down"
    if any(k in s for k in ["activate", "activator", "agonist", "up", "induce", "increase", "restore", "positive"]):
        return "up"
    return "unknown"


def create_drug_template(out_dir: Path) -> Path:
    template = pd.DataFrame({
        "drug": ["ExampleDrugA", "ExampleDrugB"],
        "gene": ["EGFR", "TP53"],
        "action": ["inhibit", "activate"],
        "source": ["template", "template"],
        "evidence": ["replace with real drug-target source", "replace with real drug-target source"],
    })
    path = out_dir / "drug_target_template.csv"
    template.to_csv(path, index=False)
    return path


def load_drug_targets(path: Optional[str], out_dir: Path) -> pd.DataFrame:
    if path is None or not Path(path).exists():
        template = create_drug_template(out_dir)
        print("[WARN] No DRUG_TARGET_FILE found/provided.")
        print("[WARN] Drug layer will output empty results. Template created:", template)
        return pd.DataFrame(columns=["drug", "gene", "action", "source", "evidence"])

    df = safe_read_table(Path(path))
    lower = {c.lower(): c for c in df.columns}

    drug_col = None
    for c in ["drug", "drug_name", "compound", "chemical", "agent", "name"]:
        if c in lower:
            drug_col = lower[c]
            break

    gene_col = None
    for c in ["gene", "target", "target_gene", "gene_symbol", "symbol", "hugo_symbol"]:
        if c in lower:
            gene_col = lower[c]
            break

    action_col = None
    for c in ["action", "effect", "interaction", "mechanism", "moa"]:
        if c in lower:
            action_col = lower[c]
            break

    if drug_col is None or gene_col is None:
        raise ValueError("Drug target file must contain drug and gene/target columns.")

    out = pd.DataFrame({
        "drug": df[drug_col].astype(str),
        "gene": df[gene_col].astype(str).map(normalize_gene_symbol),
    })
    out["action"] = df[action_col].astype(str).map(normalize_action) if action_col else "unknown"

    if "source" in lower:
        out["source"] = df[lower["source"]].astype(str)
    else:
        out["source"] = str(Path(path).name)

    if "evidence" in lower:
        out["evidence"] = df[lower["evidence"]].astype(str)
    else:
        out["evidence"] = ""

    out = out.dropna(subset=["drug", "gene"])
    out = out[(out["drug"].astype(str).str.len() > 0) & (out["gene"].astype(str).str.len() > 0)]
    out = out.drop_duplicates()
    print(f"[INFO] Loaded drug-target rows: {out.shape[0]}, drugs={out['drug'].nunique()}")
    return out


def desired_action_for_module(direction_class: str) -> str:
    s = str(direction_class)
    if s == "aberrantly_activated_in_cancerized_state":
        return "down"
    if s == "missing_good_state_or_cancerized_suppressed_BP":
        return "up"
    return "unknown"


def action_factor(drug_actions: Set[str], desired: str) -> Tuple[float, str]:
    if desired == "unknown":
        return UNKNOWN_ACTION_FACTOR, "desired_unknown"

    if not drug_actions or drug_actions == {"unknown"}:
        return UNKNOWN_ACTION_FACTOR, "drug_action_unknown"

    if desired in drug_actions:
        return ACTION_MATCH_BONUS, "action_match"

    opposite = "up" if desired == "down" else "down"
    if opposite in drug_actions:
        return ACTION_MISMATCH_PENALTY, "action_mismatch"

    return UNKNOWN_ACTION_FACTOR, "action_unclear"


def build_module_reversal_targets(module_summary: pd.DataFrame) -> pd.DataFrame:
    if module_summary is None or module_summary.empty:
        return pd.DataFrame()

    out = module_summary.copy()
    out["desired_drug_action"] = out["dominant_direction_class"].map(desired_action_for_module)
    out["reversal_interpretation"] = np.where(
        out["desired_drug_action"] == "down",
        "Suppress/reverse aberrantly activated cancerized-state module",
        np.where(
            out["desired_drug_action"] == "up",
            "Restore missing good-state/cancer-suppressed module",
            "Direction uncertain; use as monitoring/signature module"
        )
    )

    out["module_reversal_priority"] = (
        q_rank_score(out["mean_DualD_priority_score"], True) * 0.45
        + q_rank_score(out["n_Tier1_core_dual_D"], True) * 0.25
        + q_rank_score(out.get("mean_h_score", pd.Series(0, index=out.index)), True) * 0.20
        + q_rank_score(out["BP_module_size"], True) * 0.10
    )
    out = out.sort_values("module_reversal_priority", ascending=False).reset_index(drop=True)
    return out


def compute_drug_module_overlap(drug_targets: pd.DataFrame, module_targets: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    if drug_targets is None or drug_targets.empty or module_targets is None or module_targets.empty:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    ...
    drug_group = {}
    for drug, sub in drug_targets.groupby("drug"):
        gene_to_actions = {}
        for _, r in sub.iterrows():
            g = normalize_gene_symbol(r["gene"])
            gene_to_actions.setdefault(g, set()).add(str(r.get("action", "unknown")))
        drug_group[drug] = gene_to_actions

    for _, m in module_targets.iterrows():
        module_id = m["BP_module_id"]
        module_name = m["BP_module_name"]
        desired = m.get("desired_drug_action", "unknown")
        module_genes = set()
        for g in str(m.get("module_genes", "")).split(";"):
            g = normalize_gene_symbol(g)
            if g:
                module_genes.add(g)
        ...
        for drug, gene_actions in drug_group.items():
            drug_genes = set(gene_actions.keys())
            overlap = sorted(module_genes & drug_genes)
            if len(overlap) < MIN_DRUG_TARGET_OVERLAP:
                continue

            actions = set()
            for g in overlap:
                actions |= set(gene_actions.get(g, {"unknown"}))
            actions = {normalize_action(a) for a in actions}

            af, action_status = action_factor(actions, desired)
            overlap_score = len(overlap) / math.sqrt(max(len(module_genes), 1))

            module_priority = float(m.get("module_reversal_priority", m.get("mean_DualD_priority_score", 0)))
            h_bonus = 1.0
            if pd.notna(m.get("mean_h_score", np.nan)):
                h_bonus = 1.0 + min(float(m.get("mean_h_score", 0)), 50.0) / 100.0

            score = module_priority * overlap_score * af * h_bonus

            row = {
                "drug": drug,
                "BP_module_id": module_id,
                "BP_module_name": module_name,
                "dominant_direction_class": m.get("dominant_direction_class", ""),
                "desired_drug_action": desired,
                "drug_target_action_on_overlap": ";".join(sorted(actions)),
                "action_match_status": action_status,
                "module_gene_count": int(len(module_genes)),
                "drug_target_count": int(len(drug_genes)),
                "overlap_n": int(len(overlap)),
                "overlap_genes": ";".join(overlap),
                "module_reversal_priority": module_priority,
                "drug_module_overlap_score": overlap_score,
                "drug_module_reversal_score": score,
                "top_BP_terms": m.get("top_BP_terms", ""),
            }
            rows.append(row)

            if action_status == "action_mismatch":
                risk_rows.append({
                    "drug": drug,
                    "BP_module_id": module_id,
                    "BP_module_name": module_name,
                    "risk_type": "opposite_to_desired_module_direction",
                    "desired_drug_action": desired,
                    "drug_target_action_on_overlap": ";".join(sorted(actions)),
                    "overlap_n": int(len(overlap)),
                    "overlap_genes": ";".join(overlap),
                    "risk_score": overlap_score * module_priority,
                })
    ...
    if overlap_df.empty:
        return overlap_df, pd.DataFrame(), risk_df

    # Aggregate drug-level TTU score.
    agg_rows = []
    for drug, sub in overlap_df.groupby("drug"):
        total_raw = sub["drug_module_reversal_score"].sum()
        n_modules = sub["BP_module_id"].nunique()
        n_core_modules = sub.loc[sub["action_match_status"].eq("action_match"), "BP_module_id"].nunique()
        risk_score = 0.0
        n_risk = 0
        if not risk_df.empty:
            rsub = risk_df[risk_df["drug"] == drug]
            risk_score = float(rsub["risk_score"].sum()) if not rsub.empty else 0.0
            n_risk = int(rsub["BP_module_id"].nunique()) if not rsub.empty else 0

        penalty = 1.0 / (1.0 + RISK_PENALTY_PER_MODULE * n_risk + risk_score * 0.01)
        final_score = total_raw * penalty

        top_modules = (
            sub.sort_values("drug_module_reversal_score", ascending=False)
            .head(10)["BP_module_name"]
            .astype(str)
            .tolist()
        )
        ...
        agg_rows.append({
            "drug": drug,
            "TTU_raw_reversal_score": float(total_raw),
            "TTU_risk_penalty_factor": float(penalty),
            "TTU_final_score": float(final_score),
            "matched_module_count": int(n_modules),
            "action_match_module_count": int(n_core_modules),
            "risk_module_count": int(n_risk),
            "total_overlap_gene_hits": int(sub["overlap_n"].sum()),
            "top_matched_modules": ";".join(top_modules),
            "top_overlap_genes": ";".join(
                sorted(set(";".join(sub["overlap_genes"].astype(str)).split(";")) - {""})
            ),
        })

    drug_scores = pd.DataFrame(agg_rows).sort_values("TTU_final_score", ascending=False).reset_index(drop=True)
    return overlap_df.sort_values("drug_module_reversal_score", ascending=False), drug_scores, risk_df
...
def make_figures(out_dir: Path, dphy: pd.DataFrame, dclinical: pd.DataFrame, dual: pd.DataFrame,
                 module_targets: pd.DataFrame, drug_scores: pd.DataFrame, dclinical_info: Dict) -> None:
    if not HAS_MPL:
        return
    ...
    # Top BP modules
    if module_targets is not None and not module_targets.empty:
        top = module_targets.head(20)
        plt.figure(figsize=(10, max(5, 0.30 * len(top) + 2)))
        y = np.arange(len(top))
        plt.barh(y, top["module_reversal_priority"])
        plt.yticks(y, top["BP_module_name"])
        plt.xlabel("Module reversal priority")
        plt.title("Top BP-state reversal modules")
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig(fig_dir / "FIG05_top_BP_reversal_modules.png", dpi=300)
        plt.close()
    ...
    # Top drugs
    if drug_scores is not None and not drug_scores.empty:
        top = drug_scores.head(20)
        plt.figure(figsize=(10, max(5, 0.32 * len(top) + 2)))
        y = np.arange(len(top))
        plt.barh(y, top["TTU_final_score"])
        plt.yticks(y, top["drug"].astype(str).str[:50])
        plt.xlabel("TTU final score")
        plt.title("Top TTU candidate drugs")
        plt.gca().invert_yaxis()
        plt.tight_layout()
        plt.savefig(fig_dir / "FIG07_top_TTU_candidate_drugs.png", dpi=300)
        plt.close()
...
def run_pipeline() -> Path:
    ...
    config = {
        ...
        "CIVIC_ACCEPTED_FILE": CIVIC_ACCEPTED_FILE,
        "CIVIC_FEATURE_FILE": CIVIC_FEATURE_FILE,
        "COSMIC_CENSUS_FILE": COSMIC_CENSUS_FILE,
        "ONCOKB_CANCER_GENE_FILE": ONCOKB_CANCER_GENE_FILE,
    ...
    labels_out["label_name"] = np.where(labels_out["label"] == 1, "cancerized_or_positive", "good_or_negative")
    ...
    h.to_csv(out_dir / "05_h_layer_biological_anchoring.csv", index=False)
    ...
    # Reversal targets
    module_targets = build_module_reversal_targets(module_summary)
    module_targets.to_csv(out_dir / "09_BP_module_reversal_targets.csv", index=False)

    # Drug / TTU
    drug_targets = load_drug_targets(DRUG_TARGET_FILE, out_dir)
    drug_targets.to_csv(out_dir / "09b_loaded_drug_targets_normalized.csv", index=False)

    drug_module_overlap, drug_scores, risk_modules = compute_drug_module_overlap(drug_targets, module_targets)
    drug_module_overlap.to_csv(out_dir / "10_drug_BP_module_overlap.csv", index=False)
    drug_scores.head(TOP_N_DRUG_OUTPUT).to_csv(out_dir / "11_TTU_candidate_drug_scores.csv", index=False)
    risk_modules.to_csv(out_dir / "12_TTU_drug_risk_modules.csv", index=False)

    # Figures
    make_figures(out_dir, dphy, dclinical, dual, module_targets, drug_scores, dclinical_info)
    ...
    run_summary = {
        ...
        "n_BP_reversal_modules": int(module_targets.shape[0]) if module_targets is not None else 0,
        "n_drugs_loaded": int(drug_targets["drug"].nunique()) if not drug_targets.empty else 0,
        "n_drug_module_overlaps": int(drug_module_overlap.shape[0]) if drug_module_overlap is not None else 0,
        "n_TTU_candidate_drugs": int(drug_scores.shape[0]) if drug_scores is not None else 0,
        "interpretation": (
            "D-PHY captures local BP-state deviation; D-Clinical captures global clinical/cancer-state contribution. "
            "Dual-D selected BPs are reconstructed into BP-state modules, annotated with cancer-gene anchoring, "
            "mapped to desired reversal directions, and connected to drug-target overlap for TTU scoring."
        ),
    }
    ...
    # Excel workbook
    try:
        with pd.ExcelWriter(out_dir / "14_DUALD_FULL_summary.xlsx", engine="openpyxl") as writer:
            ...
            module_targets.to_excel(writer, sheet_name="module_reversal_targets", index=False)
            drug_module_overlap.head(5000).to_excel(writer, sheet_name="drug_module_overlap", index=False)
            drug_scores.head(TOP_N_DRUG_OUTPUT).to_excel(writer, sheet_name="TTU_drug_scores", index=False)
            risk_modules.head(5000).to_excel(writer, sheet_name="drug_risk_modules", index=False)
    except Exception as e:
        print("[WARN] Excel export failed, CSV outputs are still available:", e)

    # README
    readme = f"""AIDO DUAL-D FULL PIPELINE V1.1
================================
...
BP reversal modules: {module_targets.shape[0] if module_targets is not None else 0}

Drug targets loaded: {drug_targets['drug'].nunique() if not drug_targets.empty else 0}
Drug-module overlaps: {drug_module_overlap.shape[0] if drug_module_overlap is not None else 0}
TTU candidate drugs: {drug_scores.shape[0] if drug_scores is not None else 0}
...
D-PHY:
    Local single-BP state deviation.

D-Clinical:
    Global clinical/cancer-state contribution in BP space.

Dual-D:
    Union selected BP pool is passed to BP-Enrichment / BP-module reconstruction.
    Intersection/core BPs are highest-confidence.

BP-state reversal:
    cancerized_up modules should be suppressed/down-regulated.
    missing_good_state/cancerized_suppressed modules should be restored/up-regulated.

Drug layer V1:
    Drug-target overlap prototype.

Version note:
    V1.1 fixes the percentile-ranking direction used for D-PHY, D-Clinical,
    h-score, and TTU ranking composites. Use V1.1 for final results.
    If action is available, action consistency is used.
    If no drug-target file is supplied, drug_target_template.csv is created.
"""
    (out_dir / "README_DUALD_FULL_PIPELINE.txt").write_text(readme, encoding="utf-8")

    # ZIP
    zip_path = out_dir.parent / f"{out_dir.name}.zip"
    zip_dir(out_dir, zip_path)

    print("\n[DONE] AIDO DUAL-D FULL PIPELINE completed.")
